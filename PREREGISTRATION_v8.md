...
**Git commit at freeze**: 88590fcc59dc1845a4e747b7160da2f68d54afb5

## Hypothesis

H8 : conditionnellement à un premier tour strictement commun, la politique
...
H8 est évaluée séparément sur trois modèles producteurs locaux et soutenue
...
appels d'outil jusqu'à épuiser son budget sans rendre de verdict. V7 a remplacé
cette orchestration par un evidence pack déterministe, dont les tests hors
modèle ont réussi. Son premier appel Q0 a toutefois été interrompu par le délai
du lanceur externe après journalisation de la requête et avant toute réponse ;
`docs/P7_V7_STATUS.md` conserve les preuves de
`V7_ABORTED_BEFORE_CALIBRATION`. Aucun cas de calibration ou tenu n'a été lu.

V8 conserve intégralement hypothèse, données, seed, modèles, fixtures, prompts,
rubrique, seuils et verdicts substantifs V7. Elle corrige uniquement le cycle
de vie du processus : le lancement détaché est éprouvé hors campagne avant le
verrou, puis le runner expérimental est surveillé sans être tué par le retour
du contrôleur. Le programme rassemble et vérifie les données ; chaque juge ne
fait qu'une comparaison sémantique structurée dans chaque ordre.
...
Avant calibration, Q0 doit être vraie. Son échec arrête V8 avec le statut
`V8_ABORTED_BEFORE_CALIBRATION` et laisse H8 non testée :
...
vraie. Son échec arrête V8 avec `V8_ABORTED_BEFORE_HELDOUT`, laisse H8 non
...
Cette table est un calcul de design sans donnée V8. Elle interdit d'interpréter
`NOT_SUPPORTED` comme preuve d'un effet exactement nul.

## Verdict logic

Q0 et Q1 sont des portes globales préalables. Si l'une échoue, H8 reste
`H8_UNTESTED_IN_V8`.
...
- `H8_INCONCLUSIVE_FOR_MODEL` si au moins une porte structurelle échoue ;
- `H8_NOT_SUPPORTED_FOR_MODEL` si les portes structurelles passent mais au
  moins une porte opérationnelle échoue, même si le panel est ensuite trop
  indécis pour mesurer la qualité ;
- `H8_INCONCLUSIVE_FOR_MODEL` si les portes structurelles et opérationnelles
  passent mais C3 ou C4 échoue ;
- `H8_SUPPORTED_FOR_MODEL` si les portes structurelles, opérationnelles, du
  panel et d'effet passent toutes ;
- `H8_NOT_SUPPORTED_FOR_MODEL` si les portes structurelles, opérationnelles et
  du panel passent mais C5 échoue.

Global, avec N = 3 producteurs et seuil M = 2 :

- `H8_SUPPORTED_IN_V8` si au moins deux producteurs sont `SUPPORTED` ;
- `H8_NOT_SUPPORTED_IN_V8` si au moins deux producteurs sont `NOT_SUPPORTED` ;
- `H8_INCONCLUSIVE_IN_V8` dans tous les autres cas.
...
- échec Q0 : corriger uniquement l'instrument dans V9 ; calibration et tenu
...
- échec Q1 : les 12 cas de calibration deviennent données de développement ;
  le tenu reste vierge et V9 peut élargir ou modifier la calibration ;
- `INCONCLUSIVE` après ouverture : le tenu V8 devient définitivement ouvert et
...
- `NOT_SUPPORTED` : la politique adaptative n'est pas promue comme défaut, mais
  ses traces deviennent données de diagnostic pour une politique V9 distincte ;
- `SUPPORTED` : seule une réplication ou ablation préinscrite est autorisée
  avant toute promotion.

Indépendamment de H8, l'evidence pack et le harnais peuvent être livrés comme
...
ne change aucun verdict scientifique V8.

Les contraintes gelées valent exclusivement pour la revendication
confirmatoire H8. Elles ne deviennent pas des invariants permanents de Lyra :
une V9 peut remplacer politique, knobs, rubrique, panel ou seuils si elle
formule une hypothèse distincte, conserve la provenance des résultats V8 et
utilise un nouveau jeu tenu. Ainsi, un échec V8 ferme une revendication, pas le
...
de verdict, V8 est annulée et V9 préinscrite.
...
Avant tout verrou expérimental V8, le même chemin de lancement détaché doit
réussir un smoke de cycle de vie sans Ollama, sans corpus et sans fixture Q0 :
le parent rend la main, le processus enfant écrit un marqueur de départ puis un
marqueur de fin, et le moniteur observe sa terminaison avec code zéro. Ce smoke
est un contrôle d'ingénierie jetable et ne constitue pas un appel V8.

Le runner V8 est ensuite lancé une seule fois comme processus détaché, fenêtre
cachée, avec stdout et stderr séparés. Il acquiert exclusivement le verrou de
phase avant le premier appel. Le contrôleur ne peut que lire PID, journaux et
artefacts ; il ne tue ni ne relance le processus. Chaque `call_started` est
écrit et synchronisé avant envoi. Une interruption après verrou rend l'appel
`INVALID` et la phase n'est jamais rejouée.
...
déploiement. Qwen et GLM ne sont pas producteurs en V8. Un ancrage humain du
