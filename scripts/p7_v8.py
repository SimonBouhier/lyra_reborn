#!/usr/bin/env python
"""Runner P7 V8. La seule phase actuellement exposée est Q0 synthétique."""
...
PREREG_FREEZE_COMMIT = "88590fcc59dc1845a4e747b7160da2f68d54afb5"
...
def _acquire_q0_lock(output_root: Path, run_id: str) -> Path:
    """Interdit toute seconde tentative Q0 pour le même gel V8."""
    output_root.mkdir(parents=True, exist_ok=True)
    lock_path = output_root / f"p7_v8_q0_{PREREG_FREEZE_COMMIT}.lock"
    payload = _canonical(
        {
...
def run_q0(base_url: str, timeout: int, output_root: Path) -> int:
    ...
    run_dir = output_root / f"p7_v8_q0_{stamp}"
    ...
    manifest = {
        ...
        "phase": "Q0",
        "synthetic_only": True,
        "preregistration": "PREREGISTRATION_v8.md",
        "preregistration_freeze_commit": PREREG_FREEZE_COMMIT,
        "seed": GLOBAL_SEED,
    ...
    summary = {
        ...
        "run_id": run_dir.name,
        "status": "Q0_PASSED" if passed else "V8_ABORTED_BEFORE_CALIBRATION",
        "h7": "UNTESTED",
        "calls_planned": 12,
...
def run_lifecycle_smoke(output_dir: Path, delay_seconds: float) -> int:
    """Prouve que l'enfant détaché survit au retour du contrôleur, sans Ollama."""
    if delay_seconds <= 0:
        raise ValueError("delay_seconds must be positive")
    output_dir.mkdir(parents=True, exist_ok=False)
    started = {
        "schema_version": "lyra.p7.lifecycle-smoke.v1",
        "pid": os.getpid(),
...
def main() -> int:
    ...
    parser.add_argument("phase", choices=("lifecycle-smoke", "q0"))
