#!/usr/bin/env python
"""Run the diagnostic-only Gemma 3 admission bench for P7 (banc A, meta rule)."""
...
def _acquire_lock(output_root: Path, run_id: str) -> Path:
    ...
    path = output_root / f"p7_gemma3_admission_{PROTOCOL_COMMIT}.lock"
    payload = {
        "schema_version": "lyra.p7.gemma3-admission-lock.v1",
        "protocol_commit": PROTOCOL_COMMIT,
        "meta_rule_commit": META_RULE_COMMIT,
        "run_id": run_id,
...
def run(base_url: str, timeout: int, output_root: Path) -> int:
    ...
    run_dir = output_root / f"p7_gemma3_admission_{stamp}"
    _acquire_lock(output_root, run_dir.name)
    (run_dir / "calls").mkdir(parents=True, exist_ok=False)
    journal = run_dir / "journal.jsonl"
    manifest = {
        "schema_version": "lyra.p7.gemma3-admission-run.v1",
        "run_id": run_dir.name,
        "protocol": "docs/P7_GEMMA3_ADMISSION.md",
        "protocol_commit": PROTOCOL_COMMIT,
        "meta_rule_commit": META_RULE_COMMIT,
    ...
    summary = {
        "schema_version": "lyra.p7.gemma3-admission-summary.v1",
        "run_id": run_dir.name,
        "protocol_commit": PROTOCOL_COMMIT,
        "meta_rule_commit": META_RULE_COMMIT,
